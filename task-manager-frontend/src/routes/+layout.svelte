...
{#if page.url.pathname !== '/register' && page.url.pathname !== '/login'}
	<Navbar>
		...
		<div class="order-2">
			{#if data.isAuth}
				<form action="/login?/logout&redirectTo={page.url.pathname}" method="POST" use:enhance>
					<Button type="submit" color="dark">Logout</Button>
				</form>
			{:else}
				<Button color="dark" onclick={openModal}>Login</Button>
			{/if}
			...
			<Card class="w-full rounded-none border-none p-4 shadow-none sm:p-6 md:p-8">
				...
				>
					...
					<div class="text-center text-sm font-medium text-gray-500 dark:text-gray-300">
						...
						<button
							type="button"
							class="text-gray-700 underline hover:underline dark:text-white"
							onclick={() => {
								closeModal();
